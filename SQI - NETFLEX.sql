...
CREATE TABLE Users(
     ...
     plan ENUM('Basic','Standard', 'Permium') DEFAULT 'Basic'
);
CREATE TABLE Movies(
...
('Jane Smith','jane.smith@exmple.com','2024-01-15','Standard'),
...
(3,3,'Good Show,but slow at times.',3.5,'2024-02-16');
...
-- 5. Find the name of the user(s)who rated movie the highest among all reviews:
SELECT U.name
FROM Users U	
...
-- 10.Fetch all users who have reviewed a movie but have not watched it completely (complete percentage < 100)
...
JOIN Users U ON W.user_id = U.user_id
WHERE U.name = 'John Doe';
-- 12.Retrive all users who have reviewed the movie Stranger Things, including their review text and rating:
SELECT U.name,R.review_text,R.rating
FROM Users U
JOIN Reviews R ON U.user_id = R.user_id
JOIN Movies M ON R.movie_id = M.movie-id
WHERE M.title = 'Stranger Things';
...
JOIN WatchHistory W ON U.user_id = W.user_id
JOIN Movies M ON W.movie_id = M.movie_id;
-- 14.List all movies along with the total number of reviews and average rating for each movie,including only movies with atleast two reviews.
